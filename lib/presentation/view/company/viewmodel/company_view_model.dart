...
class CompanyViewModel extends BaseViewModel {
  ...
  final StreamController<ResourceState<Company>> getCompanyState =
      StreamController();

  CompanyViewModel({required CompanyRepository companyRepository})
      : _CompanyRepository = companyRepository;

  fetchCompanyUser(int idUser, int idCompany, String token) {
    getCompanyState.add(ResourceState.loading());

    _CompanyRepository.getCompany(idUser, idCompany, token)
